...
async function fetchQuestions(category: number): Promise<Pergunta[]> {
  ...
  try {
    const response = await axios.get(
      `https://opentdb.com/api.php?amount=10&category=${category}&language=pt&encode=url3986`
    );
    
    const perguntas = response.data.results.map((q: any) => ({
      id: Math.random().toString(36).substring(7),
      texto: decodeURIComponent(q.question),
      alternativas: shuffleArray([
        ...q.incorrect_answers.map((a: string) => decodeURIComponent(a)),
        decodeURIComponent(q.correct_answer)
      ]),
      correta: q.incorrect_answers.length // Índice após shuffle
    }));
...
export default function handler(req: NextApiRequest, res: NextApiResponseServerIO) {
  if (!res.socket.server.io) {
    ...
    io.on("connection", (socket) => {
      ...
      // Envia os temas disponíveis assim que o cliente se conecta
